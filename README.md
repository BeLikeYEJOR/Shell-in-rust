# To use the shell:

# Go to the latest release and download the zip folder for your System

## After you did that extract it and open the extracted folder -> target -> release -> Shell.exe(For Windows, could be different depending on your OS)

# or just download the shell executable for your os

# After you launched the shell -> If you are on mac it might not work so try: Settings -> privacy and security -> all the way down -> and open anyway

# And lastly run any commands you like😁

## Id recomend typing `help` to get all the commands you are able to run
